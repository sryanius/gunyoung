...
| 파일 | 크기 | 내용 |
|---|---|---|
| `ground.png` | 2048×360 | 채색 회화풍 전장 땅. **위(먼 곳)는 어둡고 흐리게, 아래(가까운 곳)는 밝고 풀·자갈·바퀴 자국 디테일.** 가로 seamless(좌우 페더 블렌드, 파이썬으로 이어 붙여 확인). 하늘 `sky.png` 와 색이 이어지게(해질녘 따뜻한 톤) |
...
- **땅 원근**: ground 위에 세로 그라데이션(위 어둡게 알파 0.35 → 아래 0) Graphics 한 장. fog 뒤층은 지평선(y≈400)에, 앞층은 y 650 근처 알파 0.12.
